const { yahooFinance, getProxyAgent } = require("./yahoo-finance-client");
const cache = require("./cache");

const PRICE_CACHE_TTL = 5 * 60; // 5 minutes in seconds for node-cache
const HISTORICAL_CACHE_TTL = 60 * 60 * 1000; // 1 hour
const API_DELAY_MS = 250; // Delay between individual historical data requests to Yahoo Finance
...
async function fetchStockData(symbols) {
  ...
  // Create the proxy agent once for all subsequent requests in this function call.
  const agent = getProxyAgent();
  ...
  for (const symbol of symbols) {
    ...
    if (cachedItem) {
      ...
      if (cache.get(cacheKey) === undefined) {
        backgroundFetchPromises.push(fetchAndCacheSingleSymbol(symbol, agent));
      }
    } else {
      symbolsToFetch.push(symbol);
    }
  ...
  if (symbolsToFetch.length > 0) {
    ...
    for (let i = 0; i < symbolsToFetch.length; i += BATCH_SIZE) {
      ...
      const batchPromises = batch.map(symbol => fetchAndCacheSingleSymbol(symbol, agent));
...
async function fetchAndCacheSingleSymbol(symbol, agent) {
  const today = new Date();
  const threeYearsAgo = new Date();
  threeYearsAgo.setFullYear(today.getFullYear() - 3);
