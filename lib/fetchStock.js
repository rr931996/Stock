...
function getProxyAgent() {
  ...
  console.log(`[PROXY] Creating proxy agent for ${PROXY_HOST}:${PROXY_PORT}`);
...
async function fetchStockData(symbols) {
  ...
  if (symbolsToFetch.length > 0) {
    const initialFetchPromises = symbolsToFetch.map(symbol => fetchAndCacheSingleSymbol(symbol, agent));
...
module.exports = {
  fetchStockData,
  getCurrentPrice,
  getProxyAgent, // Expose for testing if needed elsewhere
};
