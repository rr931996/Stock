  ...
  try {
    ...
    if (accessToken) {
      if (createdAt && isTokenExpired(createdAt)) {
        console.warn("[Upstox Fallback] Database token has expired (older than 24 hours).");
...
const getClientWithPersistedToken = async () => {
  if (process.env.UPSTOX_ACCESS_TOKEN && isTokenExpired(process.env.UPSTOX_TOKEN_CREATED_AT)) {
    console.log("[Upstox Fallback] Memory token has expired. Clearing it.");
    delete process.env.UPSTOX_ACCESS_TOKEN;
    delete process.env.UPSTOX_TOKEN_CREATED_AT;
  }
...
const fetchUpstoxCurrentPrices = async (symbols) => {
  ...
  const MARKET_QUOTE_BATCH_SIZE = 20;
  let quoteData = {};

  for (let i = 0; i < resolved.length; i += MARKET_QUOTE_BATCH_SIZE) {
    const batch = resolved.slice(i, i + MARKET_QUOTE_BATCH_SIZE);
    const instrumentKeys = batch.map((item) => item.instrument.instrument_key).join(",");
    let batchQuoteData = null;
    ...
    try {
      ...
      if (response.data?.status === "success" && response.data?.data) {
        batchQuoteData = response.data.data;
      }
    } catch (quoteErr) {
      console.warn(`[Upstox Fallback] Quote batch request failed for ${batch.length} symbols:`, quoteErr.message);
    }

    if (!batchQuoteData) {
      console.warn(`[Upstox Fallback] Quote API returned no data for batch, trying LTP endpoint`);
      try {
        ...
        if (ltpResponse.data?.status === "success" && ltpResponse.data?.data) {
          batchQuoteData = ltpResponse.data.data;
        }
      } catch (ltpErr) {
        console.warn(`[Upstox Fallback] LTP batch request failed for ${batch.length} symbols:`, ltpErr.message);
      }
    }

    if (batchQuoteData) {
      Object.assign(quoteData, batchQuoteData);
    }
  }
...
const fetchUpstoxHistoricalData = async (symbols) => {
  ...
  for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
    ...
    const settled = await Promise.allSettled(batch.map((symbol) => fetchSymbolHistory(symbol)));
    ...
    settled.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        errors.push({ symbol, error: result.reason?.message || 'Failed to fetch historical data from Upstox' });
      }
    });
  }
